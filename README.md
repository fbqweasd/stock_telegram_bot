...
- KOSPI·KOSDAQ은 **네이버 국내 지수 시세를 우선 조회**하며, 두 지수 모두 당일 날짜·장 종료 상태(`CLOSE`)·정규장 종료 이후 시각이 확인된 종가로 계산합니다. 수능일 등 종료 시각이 늦춰진 날에는 제공처의 정규장 종료 시각까지 기다립니다.
- 국내 종가가 확인되지 않으면 Yahoo의 당일 확정 일봉으로 폴백합니다. [Yahoo 한국 거래소 시세는 20분 지연](https://help.yahoo.com/kb/SLN2310.html)이 있으므로 보조 경로로만 사용합니다. 두 경로 모두 미확인되거나 일부 지수가 누락되면 전송하지 않고 재시도합니다.
