...
def backtest_strategy(stock_data, initial_capital=10000, buy_threshold=1.5, sell_threshold=-1.5):
    """
    백테스트: 과거 데이터로 전략의 수익률을 검증합니다.
    
    전략 규칙:
    - score >= buy_threshold (1.5) 이면 매수
    - score <= sell_threshold (-1.5) 이면 매도
    ...
    Returns:
    {
        ...
        "sharpe_ratio": 샤프 비율 (간이),
        "trades": [...]
    }
    """
    if len(stock_data["closes"]) < 60:
        return {"error": "백테스트에 필요한 데이터가 부족합니다. (최소 60개 봉)"}
    ...
    # 지지/저항은 각 시점마다 계산해야 하므로 루프 내에서 처리
    ...
    for i in range(50, len(closes)):
        ...
        sma20 = sma_20[i]
        sma50 = sma_50[i]
        bb_upper = upper_bands[i]
        bb_lower = lower_bands[i]
        atr = atr_list[i]
        
        if None in (rsi, macd, hist, sma20, sma50, bb_upper, bb_lower, atr):
            continue
        
        price = closes[i]
        
        # 간단한 점수 계산 (실제 predict 함수와 동일 로직)
        score = 0.0
        ...
        # SMA 크로스
        if i >= 1 and sma_20[i-1] is not None and sma_50[i-1] is not None:
            if sma_20[i-1] <= sma_50[i-1] and sma20 > sma50:
                score += 1.5
            elif sma_20[i-1] >= sma_50[i-1] and sma20 < sma50:
                score -= 1.5
            elif sma20 > sma50:
                score += 1.0
            elif sma20 < sma50:
        ...
        if position == 0 and score >= buy_threshold:
            ...
            entry_price = price
            capital = 0
            trades.append({"type": "BUY", "price": price, "score": score, "index": i})
        elif position > 0:
            # 익절/손절 체크
            if price >= entry_price + 1.0 * atr:  # 익절: +1 ATR
                profit = position * price
                capital = profit
                trades.append({"type": "SELL", "price": price, "score": score, "index": i, "pnl_pct": (price - entry_price) / entry_price * 100})
                position = 0
            elif price <= entry_price - 1.5 * atr:  # 손절: -1.5 ATR
                profit = position * price
                capital = profit
                trades.append({"type": "SELL", "price": price, "score": score, "index": i, "pnl_pct": (price - entry_price) / entry_price * 100})
                position = 0
            elif score <= sell_threshold:  # 시그널 반전 매도
                profit = position * price
                capital = profit
                trades.append({"type": "SELL", "price": price, "score": score, "index": i, "pnl_pct": (price - entry_price) / entry_price * 100})
                position = 0
    ...
    if position > 0:
        ...
        capital = profit
        trades.append({"type": "SELL", "price": final_price, "score": 0, "index": len(closes) - 1, "pnl_pct": (final_price - entry_price) / entry_price * 100})
    
    # 결과 계산
    total_return = (capital - initial_capital) / initial_capital * 100 if initial_capital > 0 else 0
    
    # 승률
    closed_trades = [t for t in trades if t.get("type") == "SELL" and "pnl_pct" in t]
...
if __name__ == "__main__":
    ...
    print("\n=== 백테스트 ===")
    bt = backtest_strategy(mock_data)
    pprint.pprint(bt)